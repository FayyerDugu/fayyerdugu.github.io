...
function insertAtCursor(myField, myValue) {
  ...
  //FireFox��Chrome��
    ...
    myField.value = myField.value.substring(0, startPos) + myValue + myField
...
function popupMenu() {
  window.oncontextmenu = function (event) {
    ...
    if (el.tagName == 'A') {
      $('#menu-to').show()
      rmf.open = function () {
        if (el.href.indexOf("http://") == -1 && el.href.indexOf(
          "https://") == -1 || el.href.indexOf("yisous.xyz") != -1) {
        ...
        navigator.permissions
          ...
          .then(result => {
            if (result.state == 'granted' || result.state == 'prompt') {
              ...
              Snackbar.show({
                text: 'Let me read the clipboard��',
                pos: 'top-center',
                showAction: false,
              })
            }
  ...
  if (localStorage.getItem("mouse") == "on") {
    ...
    debounce(function () {
      new Vue({
        data: function () {
          this.$notify({
            title: "Switch to system default right menu?",
            message: "Restored to system default right menu��",
            position: 'top-left',
            offset: 50,
            showClose: true,
            type: "success",
    ...
    debounce(function () {
      new Vue({
        data: function () {
          this.$notify({
            title: "Switch to custom right menu?",
            message: "Switched to custom right menu��",
            position: 'top-left',
            offset: 50,
            showClose: true,
            type: "success",
